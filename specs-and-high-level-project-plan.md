...
export interface RasterizerBeginParams {
  ...
  clipPath?: Path2D;           // Full clip stack support
}
...
*   **Pipeline**: For every draw operation, the following sequence is applied:
    ...
    2.  The transformed geometry is clipped against the active `clipPath`.
...
*   **Clip Stack**: A stack of clip regions. Each `clip()` operation intersects the new path (aliased) with the current clip mask.
...
*   **`ImageLike`**: An interface `{ width, height, data: Uint8ClampedArray }`.
    *   **Note**: When an `ImageLike` object with 3-channel RGB8 data is provided, it will be converted to RGBA8 by appending an alpha value of **255** (fully opaque) to each pixel.
...
*   **Packaging**: For simplicity, the library will NOT use ESM and CJS, nor TypeScript type definitions.
*   **Build & Environment**:
...
*   **Goldens**: A suite of golden BMP images will be generated. The creation of this golden set is a formal deliverable (see Milestones).
*   **Acceptance Criteria**:
    1.  Full visual parity with the golden image set.
    2.  BMP output is deterministic.
    3.  All public APIs function as defined in the specification.
    4.  The library builds and runs correctly in a browser (`file:///`) and in Node.js.
