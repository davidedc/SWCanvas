 ...
 *   - fillAndStrokeCircle(cx, cy, radius)
 ...
 *   - fillAndOuterStrokeArc(cx, cy, radius, startAngle, endAngle, anticlockwise)
 *
...
(function installSWCanvasCompatPolyfills() {
    ...
    if (!proto.fillAndStrokeCircle) {
        proto.fillAndStrokeCircle = function(cx, cy, radius) {
            this.beginPath();
            this.arc(cx, cy, radius, 0, Math.PI * 2);
            this.fill();
            this.stroke();
    ...
    if (!proto.fillAndStrokeRect) {
        proto.fillAndStrokeRect = function(x, y, width, height) {
    ...
    if (!proto.fillAndStrokeRoundRect) {
        proto.fillAndStrokeRoundRect = function(x, y, width, height, radii) {
            this.beginPath();
            this.roundRect(x, y, width, height, radii);
            this.fill();
            this.stroke();
    ...
    if (!proto.fillAndOuterStrokeArc) {
        proto.fillAndOuterStrokeArc = function(cx, cy, radius, startAngle, endAngle, anticlockwise) {
            if (anticlockwise === undefined) anticlockwise = false;
            // Fill the pie slice
            this.beginPath();
            this.moveTo(cx, cy);
